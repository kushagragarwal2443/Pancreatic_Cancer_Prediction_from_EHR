...
models = ["Linear Regression", "Ridge Regression", "Multi Layer Perceptron 1", "Multi Layer Perceptron 2", "Multi Layer Perceptron 3", "Decision Tree", "Random Forest"]
corr_models = ["Linear", "Ridge", "MLP1", "MLP2", "MLP3", "DT", "RF"]
...
model_descriptions = [
    ...
    "MLPRegressor(hidden_layer_sizes=(16, 32, 64, 128, 64, 32, 16), max_iter=1000)",
    "MLPRegressor(hidden_layer_sizes=(32), max_iter=1000)",
...
query_descriptions = [
    "Cough, Phlegm, Sputum, Upper respiratory tract infection",
    "Nasal congestion, Post nasal drip, Rhinorrhea, Sinusitis, Rhinitis, Common cold",
    "Fever, Hyperthermia, Chills, Shivering, Low grade fever",
    "Shortness of breath, Wheeze, Croup, Pneumonia, Asthma, Crackles, Acute bronchitis, Bronchitis",
]
...
if st.button("Predict"):
    ...
    for labvalue in labvalues_text:
        labvalue = labvalue.strip()
        if(labvalue in chart_final_mapping):
            labvalues_vector[chart_final_mapping[labvalue]] = 1

    final_vector = np.insert(procedures_vector, 0, gender_val)
    final_vector = np.concatenate((final_vector, medications_vector))
    final_vector = np.concatenate((final_vector, labvalues_vector))

    # name = "./Models/Deaths/Deaths_PA_" + str(model_name) + ".pkl"
    # with open(name, 'rb') as file:
    #     loaded_model = pickle.load(file)
    # deaths_pred = int(scaler_y_deaths.inverse_transform(loaded_model.predict(transformed_input).reshape(1, -1)).ravel()[0])
